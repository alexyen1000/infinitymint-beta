...
exports.getDeploymentClasses =
	exports.create =
	exports.getLiveDeployments =
	exports.getDeploymentClass =
	exports.hasDeploymentManifest =
	exports.readNetworkDeployment =
	exports.getNetworkDeployment =
	exports.InfinityMintDeployment =
		void 0;
...
class InfinityMintDeployment {
	constructor(deploymentScriptLocation, key, network, project) {
		...
		if (
			fs_1.default.existsSync(
				process.cwd() + "/deploy/" + deploymentScriptLocation
			)
		)
	 ...
	 */
	reload() {
		this.deploymentScript = require("./../" +
			this.deploymentScriptLocation).default;
	}
	/**
	...
	getFilePath() {
		...
		return (
			...
			`/temp/deployments/${this.project.name}@${
				((_a = this.project.version) === null || _a === void 0
					? void 0
					: _a.version) || "1.0.0"
	...
	read() {
		...
		let result = JSON.parse(
			fs_1.default.readFileSync(this.getFilePath(), {
				encoding: "utf-8",
			})
		);
		if (
	...
	isImportant() {
		var _a;
		return (
			((_a = this.deploymentScript) === null || _a === void 0
				? void 0
				: _a.important) === true
		);
	}
	...
	save() {
		...
		if (
			...
			fs_1.default.mkdirSync(
				process.cwd() +
					`/temp/deployments/${this.project.name}@${
						((_b = this.project.version) === null || _b === void 0
							? void 0
							: _b.version) || "1.0.0"
					}/`
			);
	...
	async execute(method, args) {
		let params = Object.assign(Object.assign({}, args), {
		...
		(0, helpers_1.debugLog)(
			"executing method " + method + " on " + this.key
		);
		switch (method) {
			case "deploy":
...
const readNetworkDeployment = (contractName, network) => {
	let path =
		...
		contractName +
		".json";
	if (!fs_1.default.existsSync(path)) throw new Error(`${path} not found`);
...
const hasDeploymentManifest = (contractName, project, network) => {
	...
	network =
		...
		((_a =
			project === null || project === void 0
				? void 0
				: project.network) === null || _a === void 0
			? void 0
			: _a.name);
	...
	let path =
		...
		`/temp/deployments/${project.name}@${
			((_b = project.version) === null || _b === void 0
				? void 0
				: _b.version) || "1.0.0"
...
const getDeploymentClass = (contractName, project, network) => {
	...
	let liveDeployments = (0, exports.getLiveDeployments)(
		contractName,
		project,
		network
	);
...
const getLiveDeployments = (contractName, project, network) => {
	...
	let path =
		process.cwd() +
		`/temp/deployments/${project.name}@${
			((_a = project.version) === null || _a === void 0
				? void 0
				: _a.version) || "1.0.0"
		}/${contractName}_${network}.json`;
...
const create = (liveDeployment, deploymentScript) => {
	let project = (0, helpers_1.getProject)(
...
const getDeploymentClasses = (project, network, root) => {
	return new Promise((resolve, reject) => {
		...
		network =
			network ||
			((_a = project.network) === null || _a === void 0
				? void 0
				: _a.name);
		if (network === undefined)
			throw new Error("unable to automatically determain network");
